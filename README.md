...
guarantees the math holds up under real market conditions at scale.
...
shouldn't exist for long (bots close these gaps in milliseconds), but
...
3. **Run Bellman-Ford** from a dummy source node connected to every
   ...
   The trick: normally Bellman-Ford finds shortest paths. But if you
...
4. **Reconstruct the cycle** by walking the predecessor array back
   `V` times (guaranteed to land on the cycle by pigeonhole — there
   are only `V` nodes, so a path that long must repeat one), then
...
5. **Compute real profit**: `(e^(-sum of weights) - 1) * 100`, and only
   ...
   floating-point noise doesn't get reported as "arbitrage."
...
- **Order book depth / slippage.** We use the best bid/ask only (top
  ...
  book and get a worse average price. This tool assumes you're trading
...
```powershell
.\Release\detect.exe --live --iterations 10 --interval 15 --fee 0.0015
```
